...
タイル画像は **WebP（可逆圧縮）が既定**で、PNG も選べます。どちらも可逆なので値は劣化しません。形式は TileJSON のタイル URL の拡張子が示します（専用フィールドは持ちません。仕様 §2.2）。
...
仕様 §3.2.2 により、無効値の表し方はタイルの構成で排他的に決まります。

| | 無効値 | `invalidColor` |
|---|---|---|
| 既定（アルファ付き） | アルファ 0 | 宣言しない |
| `--no-alpha --invalid-color R G B` | 指定した色 | 宣言する |

WebP の可逆圧縮は完全に透明な画素の RGB を保存しないため、アルファ付きタイルで `invalidColor` による色一致判定は成立しません。CLI は両者の併用を拒否します。

なお、無効画素が 1 つも無いタイルはアルファチャンネルを持たない形で書かれます（容量が減り、読み手は「アルファが無い＝無効画素が無い」と解釈できます）。
